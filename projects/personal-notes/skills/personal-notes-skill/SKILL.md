...
| 字段 | 说明 | 示例 |
|------|------|------|
| `📅 YYYY-MM-DD` | 截止日期（可选） | `📅 2026-03-22` |
| 优先级 emoji | ⏫ 紧急重要 / 🔼 重要 / 🔽 低优先级 / 无=普通 | `🔼` |
| `#标签` | 上下文，`#work` / `#work/项目名` / `#life` | `#work/feed-ai-app` |

**时间推断：** 用户说"今晚20点"/"明天早上"等相对时间，结合步骤1查询到的当前时间转换为绝对时间写入 `📅` 字段。
...
**时间排序规则：** 同一日期节内，有 `📅` 截止日期的条目按截止日期升序排列，无截止日期的条目排在后面，同一截止日期内按优先级排序（⏫ > 🔼 > 无 > 🔽）。

**归档规则：**
- `Personal/todo.md` 只保留 active（未完成）任务
- 已完成的旧任务（早于当前日期7天）归档到 `Personal/todo-done/YYYY-MM.md`
- 用户说"归档todo"/"清理todo"时执行归档
- Claude 发现有 `[x]` 条目早于7天前时，可主动建议归档

**视图：** `Personal/todo-dashboard.md` 提供今日聚焦、四象限、按项目等视图（依赖 Tasks 插件）。
...
<!-- Evolution: 2026-03-18 | source: brainstorm-todo-structure | pattern: tasks_plugin_format + dashboard_note + monthly_archive -->
<!-- 问题：纯文本 todo 无法支持日历/四象限/项目等视图，文件随时间无限增长 -->
<!-- 修复：Tasks 插件兼容格式（📅+优先级emoji+#tag），dashboard 笔记提供多视图，已完成条目归档到 todo-done/YYYY-MM.md -->
